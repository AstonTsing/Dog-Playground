...
class Joystick(open_duck_mini_v2_base.OpenDuckMiniV2Env):
    ...
    def _post_init(self) -> None:
        ...
        hip_ids = [
            idx for idx, j in enumerate(constants.JOINTS_ORDER_NO_HEAD) if "hip_joint" in j
        ]
        thigh_ids = [
            idx for idx, j in enumerate(constants.JOINTS_ORDER_NO_HEAD) if "thigh_joint" in j
        ]
        calf_ids = [
            idx for idx, j in enumerate(constants.JOINTS_ORDER_NO_HEAD) if "calf_joint" in j
        ]
        qpos_noise_scale[hip_ids] = self._config.noise_config.scales.hip_pos
        qpos_noise_scale[thigh_ids] = self._config.noise_config.scales.knee_pos
        qpos_noise_scale[calf_ids] = self._config.noise_config.scales.ankle_pos
    ...
    def reset(self, rng: jax.Array) -> mjx_env.State:
        ...
        qvel = jp.zeros(self.mjx_model.nv)#33333
    ...
    def sample_command(self, rng: jax.Array) -> jax.Array:
        ...
        cmd = jp.array([lin_vel_x, lin_vel_y, ang_vel_yaw])
