...
def export_onnx(
    ...
    print(f"Built TF policy network in {time.time() - export_start:.2f}s")
    ...
    print(f"Transferred weights in {time.time() - export_start:.2f}s")
    ...
    first_convert_start = time.time()
    model_proto, _ = tf2onnx.convert.from_keras(
        tf_policy_network, input_signature=spec, opset=11, output_path=output_path
    )
    print(
        f"tf2onnx export to {output_path} finished in "
        f"{time.time() - first_convert_start:.2f}s"
    )
    
    # For Antoine :)
    second_convert_start = time.time()
    model_proto, _ = tf2onnx.convert.from_keras(
        tf_policy_network, input_signature=spec, opset=11, output_path="ONNX.onnx"
    )
    print(
        f"tf2onnx export to ONNX.onnx finished in "
        f"{time.time() - second_convert_start:.2f}s"
    )
    print(f"Total export_onnx time: {time.time() - export_start:.2f}s")
    return
