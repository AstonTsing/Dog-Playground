"""Base classes for Dog quadruped robot."""
...
class DogEnv(mjx_env.MjxEnv):
    """Base class for Dog quadruped environments."""
    ...
    def get_floating_base_qpos(self, data: jax.Array) -> jax.Array:
        return data[
            self._floating_base_qpos_addr : self._floating_base_qvel_addr + 7
        ]
    ...
    def exclude_backlash_joints_addr(self) -> jax.Array:
        addr = jp.array(
            [
                self._mj_model.jnt_qposadr[idx]
                for idx in self.all_joint_no_backlash_ids
            ]
        )
        return addr
    ...
    def get_all_joints_addr(self) -> jax.Array:
        addr = jp.array(
            [self._mj_model.jnt_qposadr[idx] for idx in self.all_joint_ids]
        )
        return addr
